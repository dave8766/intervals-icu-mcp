...
### Added
- **Optional GitHub OAuth for the HTTP transport** (`INTERVALS_ICU_AUTH=github`). Over stdio the operating system is the access boundary; over HTTP the URL is, and MCP defines none of its own — so a hosted deployment (Railway, Fly, a VPS) handed every caller the full tool surface under the deployer's Intervals.icu key, which is equivalent to publishing that key. `docs/remote-deployment.md` warned about this but offered only out-of-band remedies (tunnel, reverse proxy). The server now speaks the standard MCP OAuth handshake: an unauthenticated call returns `401` with a `WWW-Authenticate` header naming the protected-resource document, which names the authorization server, which supports dynamic client registration — the chain Claude and other MCP clients follow to add a remote connector without a hand-issued client ID. **The allowlist (`INTERVALS_ICU_AUTH_ALLOWED_GITHUB_USERS`) is mandatory and the server refuses to start without it:** GitHub OAuth proves a caller holds a valid GitHub token, not that they are *you*, so a provider wired up without one is a login screen every GitHub account on earth passes — security by appearance only. Configuration lives in a `RemoteAuthConfig` kept deliberately separate from `ICUConfig`, which is injected into tool context state and therefore no place for an OAuth client secret. Auth is **off by default**, so stdio installs and existing tunnel-fronted deployments are untouched; `--transport http --host 0.0.0.0` with auth disabled now prints a warning to stderr rather than starting silently.
